...
async function buildTree(suites: any[]): Promise<any[]> {
  const map = new Map(suites.map((s) => [s.id, { ...s, children: [] }]))
  const roots: any[] = []
  for (const suite of map.values()) {
    if (suite.parentId) map.get(suite.parentId)?.children.push(suite)
...
export const suiteRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', auth, async (request, reply) => {
    const { projectId, type } = request.query as { projectId?: string; type?: string }
  ...
  fastify.post('/', auth, async (request, reply) => {
    ...
    if (body.data.parentId) {
      ...
      if (!parent) return notFound(reply, 'Parent suite not found')
    }
  ...
  fastify.get('/:id', auth, async (request, reply) => {
    ...
    if (!suite) return notFound(reply)
