...
export const jiraRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/issues/:key', auth, async (request, reply) => {
    ...
    if (!process.env.JIRA_BASE_URL) return badRequest(reply, 'Jira integration not configured')

    const res = await fetch(`${process.env.JIRA_BASE_URL}/rest/api/3/issue/${key}`, {
      headers: jiraHeaders(),
    })
    if (res.status === 404) return notFound(reply, `Jira issue ${key} not found`)
    if (!res.ok) return badRequest(reply, `Jira API error: ${res.statusText}`)
  ...
  fastify.post('/link', auth, async (request, reply) => {
    ...
    if (!tc) return notFound(reply, 'Test case not found')
