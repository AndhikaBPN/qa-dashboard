...
export const reportRoutes = async (fastify) => {
    ...
    fastify.get('/bugs-by-suite', auth, async (request, reply) => {
        ...
        for (const bug of bugs) {
            totals.total++;
            totals[bug.status] = totals[bug.status] + 1;
    ...
    fastify.get('/user-activity', auth, async (request, reply) => {
        const { projectId, from, to, weeks: weeksParam } = request.query;
        ...
        function bucketIndex(date) {
            for (let i = 0; i < buckets.length; i++) {
                if (date >= buckets[i].from && date <= buckets[i].to)
                    return i;
            }
            return -1;
        }
        ...
        for (const tc of tcCreated) {
            ...
            if (i >= 0)
                userMap.get(tc.authorId)?.weeks[i] && (userMap.get(tc.authorId).weeks[i].created++);
        }
        for (const tc of tcUpdated) {
            ...
            if (i >= 0)
                userMap.get(tc.updatedById)?.weeks[i] && (userMap.get(tc.updatedById).weeks[i].updated++);
        }
        for (const exec of executions) {
            const i = bucketIndex(exec.executedAt);
            if (i >= 0)
                userMap.get(exec.executorId)?.weeks[i] && (userMap.get(exec.executorId).weeks[i].executed++);
        }
        for (const bug of bugs) {
            const i = bucketIndex(bug.createdAt);
            if (i >= 0)
                userMap.get(bug.reporterId)?.weeks[i] && (userMap.get(bug.reporterId).weeks[i].defects++);
        }
        const result = Array.from(userMap.entries()).map(([userId, u]) => {
            const totals = u.weeks.reduce((acc, w) => ({ created: acc.created + w.created, updated: acc.updated + w.updated, executed: acc.executed + w.executed, defects: acc.defects + w.defects }), { created: 0, updated: 0, executed: 0, defects: 0 });
            return { userId, userName: u.name, weeks: u.weeks, totals };
        });
        const weekTotals = buckets.map((_, i) => result.reduce((acc, u) => ({ created: acc.created + u.weeks[i].created, updated: acc.updated + u.weeks[i].updated, executed: acc.executed + u.weeks[i].executed, defects: acc.defects + u.weeks[i].defects }), { created: 0, updated: 0, executed: 0, defects: 0 }));
        const overallTotals = weekTotals.reduce((acc, w) => ({ created: acc.created + w.created, updated: acc.updated + w.updated, executed: acc.executed + w.executed, defects: acc.defects + w.defects }), { created: 0, updated: 0, executed: 0, defects: 0 });
        return ok(reply, {
    ...
    fastify.get('/coverage', auth, async (request, reply) => {
        ...
        for (const tc of testCases) {
            const p = tc.priority;
            if (!byPriority[p])
                byPriority[p] = { total: 0, executed: 0 };
            byPriority[p].total++;
            const lastStatus = tc.executions[0]?.status;
            if (lastStatus && lastStatus !== 'NOT_RUN')
                byPriority[p].executed++;
        }
        const total = testCases.length;
