...
export const executionRoutes = async (fastify) => {
    const auth = { preHandler: [fastify.authenticate] };
    fastify.get('/', auth, async (request, reply) => {
        const { testRunId, testCaseId } = request.query;
        if (!testRunId && !testCaseId)
            return badRequest(reply, 'testRunId or testCaseId required');
    ...
    fastify.put('/:id', auth, async (request, reply) => {
        ...
        const execution = await prisma.execution.update({
            ...
            data: {
                ...
                evidence: body.data.evidence ? body.data.evidence : undefined,
